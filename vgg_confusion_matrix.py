...
from application.models.vgg19 import vgg19 as vgg19
...
parser = argparse.ArgumentParser(
    prog="Draw Confusion Matrix", description="Draw Confusion Matrix for the vgg19")
...
parser.add_argument('--showPic', metavar='showPic', type=bool,
                    default=False, help='Show patch of picture each epoch')
...
# read one picture
batch = np.load("DataSets/{0}/{0}_dataTest.npy".format(classes_name))
...
with tf.device('/device:cpu:0'):
    ...
    vgg = vgg19.Vgg19(
        "Weights/VGG19_{}.npy".format(classes_name))  # set the path
    ...
    for i in range(int(batche_num/batch_size)):
        ...
        if args.showPic:
            picShow(batch[:min_batch], labels[:min_batch], classes, None, prob)
        conf_mat += confusion_matrix(prob, tru, classes)
        print("batch :{}".format(i))
    np.save('Data/cf_mat_{}.npy'.format(classes_name), conf_mat)
    draw_confusion_matrix(
        conf_mat, classes, save_as="cf_mat_{}".format(classes_name))
    draw_table(conf_mat, classes,
               save_as="table_precision_{}".format(classes_name))
